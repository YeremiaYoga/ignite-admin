...
export default function LootTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API}/foundry/loots`, {
        method: "GET",
        headers: {
      ...
      if (!res.ok) {
        console.error("❌ Failed to load loots:", res.status, await res.text());
        return;
      }
      ...
      console.error("❌ Failed to load loots:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    const ok = confirm("Delete this loot?");
    if (!ok) return;

    try {
      const res = await fetch(`${API}/foundry/loots/${id}`, {
        method: "DELETE",
        headers: {
          ...getAuthHeader(),
        },
  ...
  const handleExport = async (id, mode) => {
    ...
    try {
      const res = await fetch(
        `${API}/foundry/loots/${id}/export?mode=${m}`,
        {
          headers: {
            ...getAuthHeader(),
          },
            ...
            {loading ? (
              ...
              data.map((x) => (
                <tr key={x.id} className="border-b border-slate-800">
                  ...
                  <td className="py-2 px-2">{x.type_value || "-"}</td>
                  <td className="py-2 px-2">{x.base_item || "-"}</td>
                  <td className="py-2 px-2 max-w-xs truncate">
                    {x.properties || "-"}
                  </td>
                  <td className="py-2 px-2">{x.rarity || "-"}</td>
                  <td className="py-2 px-2">{x.weight ?? "-"}</td>
                  <td className="py-2 px-2">{formatPrice(x.price)}</td>
                  <td className="py-2 px-2 max-w-xs truncate">
                    {x.compendium_source || "-"}
                  </td>
